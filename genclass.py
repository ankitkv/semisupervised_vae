...
class GenerativeClassifier(object):
    ...
    def _objective(self):
        ...
        ''' Labelled Datapoints '''
        self.y_lab_logits, self.x_lab = self._generate_yx(self.x_labelled_mu,
        ...
        L_lab += - self.beta * tf.nn.softmax_cross_entropy_with_logits(self.y_lab_logits,
                                                                       self.y_lab)
        ...
        def one_label_tensor(label):
            ...
            _y_ulab = tf.sparse_tensor_to_dense(
                tf.SparseTensor(indices=indices, values=values, shape=[self.num_ulab_batch,
                                                                       self.dim_y]), 0.0)
            return _y_ulab
        ...
        for label in range(self.dim_y):
            ...
            self.z_ulab, self.z_ulab_mu, self.z_ulab_lsgms = self._generate_zxy(self.x_ulab,
                                                                                _y_ulab, reuse=True)
            self.x_recon_ulab_mu, self.x_recon_ulab_lsgms = self._generate_xzy(self.z_ulab,
                                                                               _y_ulab, reuse=True)
    ...
    def train(self, x_labelled, y, x_unlabelled,
        ...
        with self.G.as_default():
            self.optimiser = tf.train.AdamOptimizer(learning_rate=learning_rate, beta1=beta1,
                                                    beta2=beta2)
