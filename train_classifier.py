...
def encode_dataset( model_path, min_std = 0.0 ):
    ...
    VAE = VariationalAutoencoder( dim_x = 28*28, dim_z = 50 )
    with VAE.session:
        ...
        enc_x_lab_mean, enc_x_lab_var = enc_x_lab_mean[ :, id_x_keep ], \
                                        enc_x_lab_var[ :, id_x_keep ]
        enc_x_ulab_mean, enc_x_ulab_var = enc_x_ulab_mean[ :, id_x_keep ], \
                                          enc_x_ulab_var[ :, id_x_keep ]
        enc_x_valid_mean, enc_x_valid_var = enc_x_valid_mean[ :, id_x_keep ], \
                                            enc_x_valid_var[ :, id_x_keep ]
        enc_x_test_mean, enc_x_test_var = enc_x_test_mean[ :, id_x_keep ], \
                                          enc_x_test_var[ :, id_x_keep ]
...
if __name__ == '__main__':
    ...
    alpha = 0.1             #Discriminatory factor (see equation (9) of
    ...
    #Uses anglpy module from original paper (linked at top) to split the dataset for
    #semi-supervised training
    train_x, train_y, valid_x, valid_y, test_x, test_y = mnist.load_numpy_split(mnist_path,
                                                                                binarize_y=True)
