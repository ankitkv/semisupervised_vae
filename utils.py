...
def tf_normal_logpdf(x, mu, log_sigma_sq):
    return (- 0.5 * logc - log_sigma_sq / 2. - tf.div(tf.square(tf.sub(x, mu)),
                                                      2 * tf.exp(log_sigma_sq)))
...
def feed_numpy_semisupervised(num_lab_batch, num_ulab_batch, x_lab, y, x_ulab):
    ...
    for i in xrange(count):
        ...
        yield [x_lab[start_lab:end_lab, :dim / 2], x_lab[start_lab:end_lab, dim / 2:dim],
               y[start_lab:end_lab], x_ulab[start_ulab:end_ulab, :dim / 2],
