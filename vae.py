...
class VariationalAutoencoder(object):
    ...
    def _objective(self):
        ...
        self.x_recon_eval, _ = self._generate_xz(self.z_sample_eval, phase=pt.Phase.test,
                                                 reuse=True)

        self.eval_log_lik = - tf.reduce_mean(tf.reduce_sum(utils.tf_binary_xentropy(
            self.x, self.x_recon_eval), 1))
    ...
    def train(self, x, x_valid,
        ...
        if save_path is None:
            self.save_path = 'checkpoints/model_VAE_{}-{}_{}.cpkt'.format(learning_rate,
                                                                          self.batch_size, time.time())
        else:
        ...
        with self.session as sess:
            ...
            for epoch in range(epochs):
                ...
                if stop_counter >= stop_iter:
                    ...
                    print('No change in validation log-likelihood for {} iterations'.format(
                        stop_iter))
